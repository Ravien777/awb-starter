...
class AWB_Asset_Loader
{
    ...
    {
        // Enqueue Font Awesome for frontend iconography.
        wp_enqueue_style('awb-font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css', [], '7.0.1', 'all');
        wp_style_add_data('awb-font-awesome', 'crossorigin', 'anonymous');
    ...
    private function enqueue_pattern_assets(): void
    {
        ...
        // Identify patterns forced globally via Header/Footer Switcher.
        $forced_patterns = [];
        ...
        if ('pattern' === $hf_header_type && ! empty($hf_header_value)) {
            $forced_patterns[] = $hf_header_value;
        }
        ...
        if ('pattern' === $hf_footer_type && ! empty($hf_footer_value)) {
            $forced_patterns[] = $hf_footer_value;
        }

        foreach (AWB_Pattern_Loader::$pattern_assets as $slug => $data) {
            $short_slug = str_replace('awb/', '', $slug);
            $is_forced  = in_array($slug, $forced_patterns, true);

            // Skip content scan if pattern is used globally as header/footer.
            if (! $is_forced) {
    ...
    public function enqueue_admin_assets(string $hook): void
    {
        ...
        if ($is_awb_page) {
            // Load Font Awesome via WP enqueue to prevent render-blocking and inline HTML violations.
            wp_enqueue_style('awb-font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css', [], '7.0.1', 'all');
            wp_style_add_data('awb-font-awesome', 'crossorigin', 'anonymous');
            ...
            wp_localize_script('awb-pattern-io', 'awbPatternIO', [
                ...
                'deleteNonce'    => wp_create_nonce('awb_delete_pattern'),
                'i18n'           => [
                    ...
                    'importProgress'  => __('Importing {current} of {total} — {name}', 'awb-starter'),
                    'importSummary'   => __('{succeeded} of {total} patterns imported successfully.', 'awb-starter'),
                    'importFailed'    => __('{failed} of {total} patterns failed.', 'awb-starter'),
                    'skipped'         => __('Skipped', 'awb-starter'),
