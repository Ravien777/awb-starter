...
class AWB_Settings
{
	...
	public function register_settings(): void
	{
		...
		$font_settings = ['awb_token_font_heading', 'awb_token_font_body', 'awb_token_font_mono'];
		foreach ($token_settings as $setting) {
			$callback = in_array($setting, $font_settings, true) 
				? [$this, 'sanitize_font_stack'] 
				: 'sanitize_text_field';
			register_setting($token_group, $setting, ['sanitize_callback' => $callback, 'default' => '']);
		}
	...
	/**
	 * Sanitizes font stack values while preserving quotes, commas, and spaces.
	 */
	public function sanitize_font_stack(string $value): string
	{
	...
	public function save_design_tokens(): void
	{
		...
		foreach ($token_settings as $setting) {
			if (isset($_POST[$setting])) {
				// Normalize input to remove potential escaping slashes.
				$value = wp_unslash($_POST[$setting]);
				$value = in_array($setting, $font_settings, true) 
					? $this->sanitize_font_stack($value) 
